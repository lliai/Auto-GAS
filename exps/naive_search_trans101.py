...
from exps.draw.vis_rank_corr import plot_rank_corr
...
from gaswot.pipeline.postprocess import (
    ...
    no_op, normalize_op, slogdet_op, trace_op, transpose_op)
...
def evaluate_proxy(gaswot_proxy, trainloader, ss_type, sample_num=50):
    ...
    print('Plotting rank correlation ...')
    x_label = 'Ground Truth SSIM'
    y_label = 'Auto-GAS Proxy Score'
    plot_rank_corr(gt_list, zc_list, x_label, y_label,
                   f'./logs/naive_search/rank_corr_{gaswot_proxy}.png')
    print(f'gt_list = {gt_list}')
    print(f'zc_list = {zc_list}')

    return pearson(gt_list, zc_list)
...
if __name__ == '__main__':
    ...
    best_proxy, best_zc = search_proxy(trainloader, args.ss_type,
                                       args.iteration, args.sample_num)

    print(f'Best gaswot_proxy(sp={best_zc}) is:')
    print(best_proxy)

    # evaluate the rank consistency of autoencoder task
    ps = evaluate_proxy(best_proxy, trainloader, args.ss_type,
                        args.sample_num * 2)

    print(f'1 Pearson of gaswot is {ps:.4f}')
    # print(f'Best gaswot_proxy is {best_proxy}')

    # evaluate the rank consistency of autoencoder task
    ps = evaluate_proxy(best_proxy, trainloader, args.ss_type,
                        args.sample_num * 2)

    print(f'2 Pearson of gaswot is {ps:.4f}')
    print(f'Best gaswot_proxy is {best_proxy}')

    # proxy1 = GasWOTProxy(s_ops=[trans_mask], h_ops=[above_mean_op], p_ops=[l2_norm_op, l1_norm_op, trace_op])

    # proxy2 = GasWOTProxy(s_ops=[trans_local_s1], h_ops=[above_median_op], p_ops=[slogdet_op, no_op, element_wise_pow_op])

    # proxy3 = GasWOTProxy(s_ops=[trans_softmax_HW], h_ops=[above_zero_op], p_ops=[element_wise_log_op, element_wise_abslog_op, hamming_op])
    ...
    # ps_list = []
    # proxy_cand = [proxy1, proxy2, proxy3]
    ...
    #     for i in range(5):
    #         # evaluate the rank consistency of autoencoder task
    #         ps = evaluate_proxy(pcand, trainloader, args.ss_type,
    #                             args.sample_num)
    #         ps_list.append(ps)
    ...
    #     print(f"For {k}, avg is {v['avg']} and std is {v['std']}")
