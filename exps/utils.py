...
def add_algo_cfg(cfg, args):
    ...
    if 'benchmark_algo' in args.__dict__.keys():
        ...
        if not (Path(args.algo_cfg_dir) / 'config.py').is_file():
            raise ImportError('config.py not found in {0}'.format(
                args.algo_cfg_dir))
...
def setup_model(cfg, device_list, rank=None, ddp=False):
    ...
    # setup optimizer
    if 'optimizer' in cfg:
        params = list(cfg['encoder'].parameters()) + list(
            cfg['decoder'].parameters())
        cfg['optimizer'] = cfg['optimizer'](params, **cfg['optimizer_kwargs'])
    elif 'g_optimizer' and 'd_optimizer' in cfg:
        g_params = list(cfg['encoder'].parameters()) + list(
            cfg['decoder'].parameters())
        d_params = list(cfg['discriminator'].parameters())
        cfg['g_optimizer'] = cfg['g_optimizer'](g_params,
                                                **cfg['g_optimizer_kwargs'])
        cfg['d_optimizer'] = cfg['d_optimizer'](d_params,
                                                **cfg['d_optimizer_kwargs'])

    # setup lr scheduler
    if 'lr_scheduler' in cfg and cfg['lr_scheduler']:
        cfg['lr_scheduler'] = cfg['lr_scheduler'](cfg['optimizer'],
                                                  cfg['num_epochs'],
                                                  cfg['warmup_epochs'])
    elif 'g_lr_scheduler' in cfg and cfg['g_lr_scheduler']:
        cfg['g_lr_scheduler'] = cfg['g_lr_scheduler'](cfg['g_optimizer'],
                                                      cfg['num_epochs'],
                                                      cfg['warmup_epochs'])
        cfg['d_lr_scheduler'] = cfg['d_lr_scheduler'](cfg['d_optimizer'],
                                                      cfg['num_epochs'],
                                                      cfg['warmup_epochs'])
...
def setup_model_darts(cfg, device_list, rank=None, ddp=False):
    cfg['model_type'].to_device(device_list, rank, ddp=ddp)
    model_weights = list(cfg['encoder'].get_weights() +
                         list(cfg['decoder'].parameters()))
    model_alpha = cfg['encoder'].get_alphas()

    # Initialize optimizers
    cfg['w_optimizer'] = cfg['w_optimizer'](model_weights,
                                            **cfg['w_optimizer_kwargs'])
    cfg['a_optimizer'] = cfg['a_optimizer'](model_alpha,
                                            **cfg['a_optimizer_kwargs'])

    # Initialize lr scheduler
    cfg['w_lr_scheduler'] = cfg['w_lr_scheduler'](cfg['w_optimizer'],
                                                  cfg['num_epochs'],
                                                  cfg['warmup_epochs'])
