...
from network import LinearLrDecay, copy_params, load_params, train, validate
...
from gaswot.utils.utils import (count_parameters_in_MB, create_logger,
                                save_checkpoint, save_is_checkpoint,
                                set_log_dir)
...
def main():
    ...
    genotype_G = np.load(os.path.join('data', 'best_dis_c10.npy'))
    genotype_D = np.load(os.path.join('data', args.genotypes_exp))
    ...
    # train loop
    for epoch in tqdm(
            range(int(start_epoch), int(args.max_epoch_D)),
            desc='total progress'):
