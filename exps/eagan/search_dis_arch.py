...
from trainer.trainer_discriminator import DisTrainer

import gaswot.dataset.datasets as datasets
from gaswot.archs.super_network import Discriminator, Generator
...
def main():
    ...
    gen_genotype = np.load(os.path.join('exps', 'best_gen_c10.npy'))
    dis_trainer = DisTrainer(args, gen_net, dis_net, gen_optimizer,
                             dis_optimizer, train_loader, gan_alg,
                             gen_genotype)
    # search discriminator
    for epoch in tqdm(
            range(int(start_epoch), int(args.epoch_discriminator)),
            desc='search discriminator'):
        if epoch > args.warmup and (epoch -
            ...
            for index, best_genotype in enumerate(best_genotypes):
                file_path = os.path.join(
                    ...
                    'best_dis_{}_{}.npy'.format(str(epoch), str(index)))
