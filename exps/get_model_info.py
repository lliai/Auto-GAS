...
from gaswot.tnb101.models.model_info import get_inference_time
...
if __name__ == '__main__':
    ...
    tasks = [
        'class_object', 'jigsaw', 'class_scene', 'room_layout', 'autoencoder',
        'normal', 'segmentsemantic'
    ]

    root_dir = str((Path(__file__).parent / '..' / '..' / '..').resolve())

    encoder_str_path = os.path.join(
        root_dir, 'transnas-bench/configs/benchmark_cfg/net_strings.json')
    ...
    save_name = os.path.join(
        root_dir,
        f'transnas-bench/benchmark_results/models_latency_{start}-{end}.json')
    for id, encoder in enumerate(encoder_strs):
        ...
        all_dict[encoder] = {}
        for task in tasks:
            args.cfg_dir = os.path.join(
                root_dir,
                'transnas-bench/configs/task_cfg/train_from_scratch/', task)
            args.encoder_str = encoder

            rank = 0
            world_size = len(device_list)
            all_dict[encoder][task] = main(
                task,
                rank=rank,
