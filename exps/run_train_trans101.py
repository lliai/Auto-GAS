...
from gaswot.dataset import get_datasets
from gaswot.log_utils import Logger, ModelDB
    ...
    try:
        ...
        logger.write(
            '--------------- {0:12} setup finished! ---------------'.format(
                'Dataloader'))
        logger.write(
            f'len(train_data) {len(train_data)}; len(train_loader) {len(train_loader)}'
        )
        logger.write(
            f'len(val_data) {len(val_data)}; len(val_loader) {len(val_loader)}'
        )
        logger.write(
            f'len(test_data) {len(test_data)}; len(test_loader) {len(test_loader)}'
        )

        # execute training
        start_train_time = time.time()
        utils.print_start_info(cfg, is_training=True)
        model_db.change_status_save(
            'running')  # ['running', 'finished', 'corrupted']
        model.evaluate_for_seed(cfg, train_loader, val_loader, test_loader,
                                logger, model_db)
        end_train_time = time.time() - start_train_time

        logger.write('time to train %d epochs: %.3f hrs' %
                     (cfg['num_epochs'], end_train_time / (60 * 60)))
        logger.write('avg time per epoch: %.3f hrs' %
        ...
        logger.write(
            f"model {args.encoder_str} in {cfg['task_name']} is corrupted!")
        logger.write(e)
        if args.ddp:
            utils.cleanup()
        raise
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'cfg_dir',
        type=str,
        default='./configs/autoencoder',
        help='directory containing config.py file')
    ...
    if args.ddp:  # DDP
        ...
        mp.spawn(
            main,
            args=(world_size, device_list, args),
            nprocs=world_size,
            join=True)
    else:
        rank = 0
