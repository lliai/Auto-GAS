...
from gaswot.dataset import load_ops
...
def classification(preds, synset, **kwargs):
    ...
    for b in range(batch_size):
        ...
        to_print_pred = 'Top 5 prediction: \n {}\n {}\n {}\n {} \n {}'.format(
            *top_5_pred)
        img_size = (256, 256) if 'img_size' not in kwargs.keys() else tuple(
            kwargs['img_size'])
        img = Image.new('RGB', img_size, (255, 255, 255))
        d = ImageDraw.Draw(img)
        fnt = ImageFont.truetype(
            str((Path(__file__).parent /
                 'DejaVuSerifCondensed.ttf').resolve()), 22)
...
def set_img_color(colors, background, img, gt, show255=False):
    for i in range(0, len(colors)):
        if i != background:
            img[np.where(gt == i)] = tuple(
                [value * 255. for value in colors[i]])
        else:
...
def seg_img(background, img, gt):
    ...
    """
    labels = ('bottle', 'chair', 'couch', 'plant', 'bed', 'd.table', 'toilet',
              'tv', 'microw', 'oven', 'toaster', 'sink', 'fridge', 'book',
              'clock', 'vase')
    colors = ('white', 'red', 'blue', 'yellow', 'magenta', 'green', 'indigo',
              'darkorange', 'cyan', 'pink', 'yellowgreen', 'black',
              'darkgreen', 'brown', 'gray', 'purple', 'darkviolet')
...
def check_if_point_in_fustrum(point, fov):
    return all([
        np.abs(math.atan(coord / point[2])) < fov / 2. for coord in point[:2]
    ])
...
def plot_bb_c(pred_corners, pred_edge, ax=None):
    ...
    for (s_idx, s), (e_idx,
                     e) in itertools.combinations(enumerate(pred_corners), 2):
        if any([
                np.isclose(np.linalg.norm(s - e), el, atol=1e-04)
                for el in pred_edge
        ]):
...
def plot_points_with_bb(pred_corners,
                        pred_edge,
                        cube_only=False,
                        fov=None,
                        space='camera',
...
def plot_room_layout(predicted,
                     cube_only=False,
                     overlay=False,
                     keep_ratio=True):
    ...
    if cube_only:
        predicted[:3] = [0, 0, -1]
        if keep_ratio:
            predicted[-3:] = 7 * predicted[-3:] / np.prod(
                predicted[-3:])**(1 / 3)
        else:
            predicted[-3:] = [8, 8, 8]
    corners_for_cam_prediction, edge_lengths_pred = get_cam_corners_and_edge_ordered(
        predicted)
    camera_space_plot = plot_points_with_bb(
        pred_corners=corners_for_cam_prediction[:, :3],
        pred_edge=edge_lengths_pred,
        cube_only=cube_only,
        fov=1,
        space='camera',
        subplot=(1, 1, 1),
...
def room_layout(preds):
    ...
    mean = np.array([
        -1.64378987e-02, 7.03680296e-02, -2.72496318e+00, 1.56155458e+00,
        -2.83141191e-04, -1.57136446e+00, 4.81219593e+00, 3.69077521e+00,
        2.61998101e+00
    ])
    std = np.array([
        0.73644884, 0.53726124, 1.45194796, 0.19338562, 0.01549811, 0.42258508,
...
def task_demo(task_name, store_path, imgs, tars, preds, **kwargs):
    ...
    if task_name in ['autoencoder', 'normal', 'inpainting', 'denoise']:
        ...
        imglist = torch.cat([
            imgs,
            semseg_imgs(tars, imgs),
            semseg_imgs(preds.squeeze(1), imgs)
        ], 0)
        ...
        imglist = torch.cat([
            imgs,
            classification(tars, synset, img_size=img_size),
            classification(preds, synset, img_size=img_size)
        ], 0)
    elif task_name in ['jigsaw']:
        permutation_set = load_ops.get_permutation_set(**kwargs)
        raw_imgs = tiles2image(imgs)
        tar_imgs = tiles2image(
            imgs,
            reverse_permutations(get_permutations(tars, permutation_set)))
        pred_imgs = tiles2image(
            imgs,
            reverse_permutations(
                get_permutations(preds.argmax(dim=-1), permutation_set)))
