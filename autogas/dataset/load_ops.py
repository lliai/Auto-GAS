...
class ColorJitter(T.ColorJitter):
    ...
    def __call__(self, sample, task_name):
        ...
        image, label = sample['image'], sample['label']
        if task_name in ['autoencoder']:
            return {'image': t(image), 'label': t(label)}
        elif task_name == 'segmentsemantic':
            return {'image': t(image), 'label': label}
        elif task_name in [
                'class_object', 'class_scene', 'room_layout', 'normal',
                'jigsaw'
        ]:
            return {'image': t(image), 'label': label}
        else:
...
def get_synset(task, selected=True, final5k=False):
    ...
    if task == 'class_scene':
        ...
        with open((Path(__file__).parent / 'class_scene_names.json').resolve(),
                  'r') as fp:
            synset_scene = [x[3:] for x in json.load(fp)]
            if selected:
                synset_scene = [
                    x for x, y in zip(synset_scene, selection) if y == 1.
                ]
        synset = synset_scene
        ...
        if selected:
            synset_object = [
                x for x, y in zip(synset_object, selection) if y == 1.
            ]
...
def get_room_layout_cam_mat_and_ranges(view_dict, make_x_major=False):
    ...
    ranges = [
        bbox_ranges['x'], -np.array(bbox_ranges['y'])[::-1], bbox_ranges['z']
    ]
    ...
    def make_world_x_major(view_dictx):
        ...
        for world_rotx in apply_90_deg_rot_k_times:
            ...
            best = min(
                best,
                (degrees_away, np.linalg.inv(world_rotx)))  # python is neat
...
def point_info2room_layout(label_path):
    ...
    def convert_world_to_cam(points, cam_mat=None):
        ...
        new_points = np.dot(homogenized_points,
                            np.linalg.inv(cam_mat).T)[:, :3]
        return new_points
    ...
    camera_matrix, bb = get_room_layout_cam_mat_and_ranges(
        data, make_x_major=True)
    ...
    bbox_cam = np.hstack(
        (cube_center, camera_matrix_euler, x_scale, y_scale, z_scale))
...
def image2tiles(img, permutation, new_dims):
    ...
    if len(permutation) != 9:
        raise ValueError(
            f'Target permutation of Jigsaw is supposed to have lenght 9, getting {len(permutation)} here'
        )
...
class MakeJigsawPuzzle(object):
    ...
    def __call__(self, sample, task_name):
        ...
        if task_name == 'jigsaw':
            ...
            if self.totensor:
                raws = torch.empty((9, 3, self.tile_dim[0], self.tile_dim[1]),
                                   dtype=torch.float32)
                images = torch.empty(
                    (9, 3, self.tile_dim[0], self.tile_dim[1]),
                    dtype=torch.float32)
