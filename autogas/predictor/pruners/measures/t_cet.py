...
)  # change bn from Fasle to True, in order to avoid lnwot changes. We will manually remove bn in compute_tcet_score set synflow WITHOUT bn.
@measure(
...
@measure(
    'tcet', bn=True, mode='none', mt='synflow'
)  # change bn from Fasle to True, in order to avoid lnwot changes. We will manually remove bn in compute_tcet_score set synflow WITHOUT bn.
def compute_tcet_score(net,
                       inputs,
                       targets,
                       loss_fn=None,
                       split_data=1,
    ...
    net1 = net.get_prunable_copy(
        bn=True)  # manually keep bn in lnwot, and remove bn in synflow
    ...
    net2 = net.get_prunable_copy(
        bn=True if mt == 'snip' else
        False)  # manually keep bn in lnwot, and remove bn in synflow
...
def compute_snr_score(net,
    ...
    for mt_score in mt_scores:
        ...
        sigma = torch.std(s)
        sigma = torch.tensor(0.) if torch.isnan(
            sigma
        ) else sigma  ###TODO: Notice, torch.std(torch.tensor([2.2]))==tensor(nan), np.std([2.2])==0.
