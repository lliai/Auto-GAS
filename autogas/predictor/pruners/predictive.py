...
def find_measures(
    ...
    # a tuple with (dataload_type = {random, grasp},
    # number_of_batches_for_random_or_images_per_class_for_grasp, number of classes)
    dataload_info,
    ...
    for k, v in measures_arr.items():
        if k in [
                'jacov', 'epe_nas', 'nwot', 'zen', 'diswot', 'entropy', 'zico',
                'bn_score', 'mixup', 'size', 'grad_angle', 'grad_conflict',
                'zico', 'mgm', 'ntk', 'condnum', 'ntk_trace', 'jacobian_trace',
                'logits_entropy', 'hessian_trace', 'orm', 'nst', 'sp1', 'at',
                'sp2', 'pdist', 'cc', 'ickd', 'meco', 'grad_hamming', 'lnwot',
                'lnwot_Kmats', 'tcet_syn_none', 'tcet_syn_log',
                'tcet_syn_log1p', 'tcet_syn_norm', 'tcet_snip_none',
                'tcet_snip_log', 'tcet_snip_log1p', 'tcet_snip_norm', 'tcet',
                'custom'
        ]:
            measure_score = v
        else:
