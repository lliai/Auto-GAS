...
PRIMITIVES = [
    'none', 'skip_connect', 'conv_1x1', 'conv_3x3', 'conv_5x5', 'dil_conv_3x3',
    'dil_conv_5x5'
]

PRIMITIVES_wo_act = [
    'conv_1x1', 'conv_3x3', 'conv_5x5', 'dil_conv_3x3', 'dil_conv_5x5'
]

PRIMITIVES_up = ['nearest', 'bilinear', 'ConvTranspose']

PRIMITIVES_down = [
    'avg_pool', 'max_pool', 'conv_3x3', 'conv_5x5', 'dil_conv_3x3',
    'dil_conv_5x5'
]
...
def alpha2genotype(alpha_normal, alpha_up, save=False, file_path=None):
    ...
    genotype = np.ones(
        (alpha_up.shape[0], alpha_up.shape[1] + alpha_normal.shape[1]),
        dtype=np.uint8) * 100
    ...
    for cell_i in range(num_cell):
        ...
        for edge_j in range(alpha_normal.shape[1]):
            genotype[cell_i][offset + edge_j] = np.argmax(
                alpha_normal[cell_i][edge_j])
    ...
    for cell_i in range(num_cell):
        ...
        if genotype[cell_i][2] + genotype[cell_i][3] == 0:
            edge_j = np.argmin([
                alpha_normal[cell_i][2 - offset][0],
                alpha_normal[cell_i][3 - offset][0]
            ])
            genotype[cell_i][offset + edge_j] = np.argmax(
                np.delete(alpha_normal[cell_i][edge_j], 0)) + 1

        # node4 of all cells must has a input feature
        if genotype[cell_i][4] + genotype[cell_i][5] + genotype[cell_i][6] == 0:
            edge_j = np.argmin([
                100, 100, alpha_normal[cell_i][2][0],
                alpha_normal[cell_i][3][0], alpha_normal[cell_i][4][0]
            ])
            genotype[cell_i][offset + edge_j] = np.argmax(
                np.delete(alpha_normal[cell_i][edge_j], 0)) + 1
...
def beta2genotype(beta_normal, beta_down, save=False, file_path=None):
    ...
    genotype = np.ones(
        (beta_normal.shape[0], beta_normal.shape[1] + beta_down.shape[1]),
        dtype=np.uint8) * 100
    ...
    for cell_i in range(num_cell):
        for edge_i in range(beta_normal.shape[1]):
            if (cell_i == 0) and (edge_i in [0, 1, 2]):
                genotype[cell_i][edge_i] = np.argmax(
                    np.delete(beta_normal[cell_i][edge_i], [5, 6]))
            else:
                genotype[cell_i][edge_i] = np.argmax(
                    beta_normal[cell_i][edge_i])

        for edge_j in range(beta_down.shape[1]):
            genotype[cell_i][offset + edge_j] = np.argmax(
                beta_down[cell_i][edge_j])
    ...
    for cell_i in range(1, num_cell):
        ...
        if genotype[cell_i][1] + genotype[cell_i][3] == 0:
            edge_i = np.argmin([
                100, beta_normal[cell_i][1][0], 100, beta_normal[cell_i][3][0]
            ])
            genotype[cell_i][edge_i] = np.argmax(
                np.delete(beta_normal[cell_i][edge_i], 0)) + 1

        # node3 of all cells must have a input feature
        if genotype[cell_i][2] + genotype[cell_i][4] == 0:
            edge_i = np.argmin([
                100, 100, beta_normal[cell_i][2][0], 100,
                beta_normal[cell_i][4][0]
            ])
            genotype[cell_i][edge_i] = np.argmax(
                np.delete(beta_normal[cell_i][edge_i], 0)) + 1

        # need node1
        if (genotype[cell_i][3] + genotype[cell_i][4] >
                0) and (genotype[cell_i][0] == 0):
            genotype[cell_i][0] = np.argmax(
                np.delete(beta_normal[cell_i][0], 0)) + 1
