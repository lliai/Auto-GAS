...
from gaswot.procedures import task_demo
...
class Timer:
    ...
    def total_time_string(self, current_step):
        now = time.time()
        avg_time_per_step = (now - self.start) / (
...
def logging(cfg,
    ...
    msg = [
        met_name + ' [%.4f]' % met_value
        for met_name, met_value in metrics.items()
    ]
    msg.insert(
        0, f'{timer.elapse_string()}'
        if training else f'[{timer.time_status(epoch + 1)}]')
...
def end_epoch_log(cfg, epoch, metrics, model_dic, model_db, logger, recorder,
                  timer):
    ...
    if cfg['task_name'] in ['class_object', 'class_scene']:
        logger.write('Highest validation acc: %.4f' %
                     recorder.max_metric('valid_top1'))
    elif cfg['task_name'] in ['autoencoder', 'normal', 'inpainting']:
        logger.write('Highest validation ssim: %.4f' %
                     recorder.max_metric('val_ssim'))
    elif cfg['task_name'] == 'segmentsemantic':
        logger.write('Highest validation mIoU: %.4f' %
                     recorder.max_metric('valid_mIoU'))
...
def get_confusion_matrix(gt_labels, pred_labels, class_num):
    ...
    for i, (gt_label, pred_label) in enumerate(zip(gt_labels, pred_labels)):
        ...
        index = (gt_label.flatten() * class_num +
                 pred_label.flatten()).astype('int32')
