...
from gaswot.tnb101.models.net_ops.norm import NaiveSyncBatchNorm
...
class FeedForwardNet(nn.Module):
    ...
    def evaluate_for_seed(self, cfg, train_loader, valid_loader, test_loader,
        ...
        for epoch in range(cfg['num_epochs']):
            # Train
            train_top1, train_top5, train_loss = self.procedure(
                epoch, cfg, train_loader, logger, timer, 'train')

            # Validation
            with torch.no_grad():
                valid_top1, valid_top5, valid_loss = self.procedure(
                    epoch, cfg, valid_loader, logger, timer, 'val')

            # Test
            with torch.no_grad():
                test_top1, test_top5, test_loss = self.procedure(
                    epoch, cfg, test_loader, logger, timer, 'test')
    ...
    def procedure(self, epoch, cfg, data_loader, logger, timer, mode):
        ...
        loss_meter, top1_meter, top5_meter = AverageMeter(), AverageMeter(
        ), AverageMeter()
        ...
        for step in range(1, len(data_loader) + 1):
            ...
            top1, top5 = get_topk_acc(
                logits, labels.argmax(dim=-1), topk=(1, 5))
    ...
    def evaluate_for_seed_darts(self, cfg, w_loader, a_loader, valid_loader,
                                test_loader, logger, model_db):
        timer = Timer(0, cfg['num_epochs'])
        recorder = RecorderMeter(cfg['metric_content'][:-1])
        ...
        for epoch in range(cfg['num_epochs']):
            ...
            if self.rank == 0 or len(self.device_list) == 1:
                ...
                arch2list = lambda l, encoder_str: encoder_str.split(
                    '-')[:2] + [f'{l[0]}_{l[1]}{l[2]}_{l[3]}{l[4]}{l[5]}']
                Astr += f"Current Alphas: {alpha};\n Early Stopped Arch: {'-'.join(arch2list(arch, cfg['encoder_str']))}"

                logger.write(Sstr + Wstr + Astr)

    def procedure_darts(self, epoch, cfg, w_loader, a_loader, logger, timer,
                        mode):
        assert mode == 'train'
        self.train(True if mode == 'train' else False)
        w_loss_meter, w_top1_meter, w_top5_meter = AverageMeter(
        ), AverageMeter(), AverageMeter()
        a_loss_meter, a_top1_meter, a_top5_meter = AverageMeter(
        ), AverageMeter(), AverageMeter()
        ...
        for step in range(1, len(w_loader) + 1):
            ...
            w_top1, w_top5 = get_topk_acc(
                w_logits, w_labels.argmax(dim=-1), topk=(1, 5))
            ...
            a_top1, a_top5 = get_topk_acc(
                a_logits, a_labels.argmax(dim=-1), topk=(1, 5))
            ...
            if epoch == 0 and step == 1:
                demo(
                    ...
                    w_logits[:3],
                    extra_msg='')
            w_loss_meter.update(w_loss)
            a_loss_meter.update(a_loss)
            w_top1_meter.update(w_top1)
            a_top1_meter.update(a_top1)
            w_top5_meter.update(w_top5)
            a_top5_meter.update(a_top5)

            if step == 1 or step % 200 == 0:
                if cfg['task_name'] == 'room_layout':
                    metrics = {
                        'w_loss': w_loss_meter.avg,
                    ...
                    metrics = {
                        'w_loss': w_loss_meter.avg,
                        'a_loss': a_loss_meter.avg,
                        'w_train_top1': w_top1_meter.avg,
                        'a_train_top1': a_top1_meter.avg,
                        'w_train_top5': w_top5_meter.avg,
                        'a_train_top5': a_top5_meter.avg
                    }
                ...
                logging(
                    ...
                    training=True,
                    step=step,
                    total_step=len(w_loader),
                    extra_message=Astr)
        return [
            w_top1_meter.avg, w_top5_meter.avg, w_loss_meter.avg,
    ...
    def to_device(self, device_list, rank=None, ddp=False):
        ...
        if len(self.device_list) > 1:
            if ddp:
                self.encoder = NaiveSyncBatchNorm.convert_sync_batchnorm(
                    self.encoder)
                self.decoder = NaiveSyncBatchNorm.convert_sync_batchnorm(
                    self.decoder)
