...
from gaswot.tnb101.models.utils import Timer, demo, end_epoch_log, logging
...
class GAN(nn.Module):
    ...
    def forward_with_features(self, x):
        feature, x = self.encoder.forward_with_features(x)
        return feature, self.decoder(x)
    ...
    def prepare_evaluate(self, cfg):
        ...
        self.scheduler_G = cfg[
            'g_lr_scheduler'] if 'g_lr_scheduler' in cfg and cfg[
                'g_lr_scheduler'] else None
        self.scheduler_D = cfg[
            'd_lr_scheduler'] if 'd_lr_scheduler' in cfg and cfg[
                'd_lr_scheduler'] else None
    ...
    def evaluate_for_seed(self, cfg, train_loader, valid_loader, test_loader,
        ...
        for epoch in range(cfg['num_epochs']):
            # Train
            g_loss, d_loss, train_l1_loss, train_ssim = self.procedure(
                epoch, cfg, train_loader, logger, self.timer, 'train')

            # Validation
            with torch.no_grad():
                val_l1_loss, val_ssim = self.procedure(epoch, cfg,
                                                       valid_loader, logger,
                                                       self.timer, 'val')

            # Test
            with torch.no_grad():
                test_l1_loss, test_ssim = self.procedure(
        ...
        for step in range(1, len(data_loader) + 1):
            ...
            if (epoch == 0 and step == 1) or (step == len(data_loader)
                ...
                demo(
                    ...
                    self.fake_B[:3],
                    extra_msg='')
            self.ssim = cfg['ssim'](self.real_B,
                                    self.fake_B.clone().float().detach())
    ...
    def update_G(self, epoch, step, total_step, mode):
        ...
        self.loss_G_GAN = self.criterionGAN(
            D_pred_fake,
            G_label_real) * self.gan_loss_weight  # aim at D(G(A)) = 1.
        self.loss_G_L1 = self.criterionL1(
            self.fake_B, self.real_B) * self.l1_weight  # aim at G(A) = B
        ...
        if len(self.device_list) > 1:
            if ddp:
                ...
                self.decoder = NaiveSyncBatchNorm.convert_sync_batchnorm(
                    self.decoder)
                self.discriminator = NaiveSyncBatchNorm.convert_sync_batchnorm(
                    self.discriminator)
                self.encoder = DDP(
