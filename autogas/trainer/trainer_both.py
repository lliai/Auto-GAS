...
class BothTrainer():
    ...
    def __init__(self, args, gen_net, dis_net, gen_optimizer, dis_optimizer,
        ...
        if args.warmup == 0:
            self.gen_genotypes = np.stack(
                [gan_alg.search_mutate() for i in range(args.num_individual)],
                axis=0)
            self.dis_genotypes = np.stack([
                gan_alg.search_mutate_dis() for i in range(args.num_individual)
            ],
                                          axis=0)
        else:
            self.gen_genotypes = np.stack(
            ...
            self.dis_genotypes = np.stack(
                [gan_alg.search_dis() for i in range(args.num_individual)],
                axis=0)
    ...
    def train(self, epoch, writer_dict, schedulers=None):
        ...
        for iter_idx, (imgs, _) in enumerate(tqdm(self.train_loader)):
            ...
            z = torch.cuda.FloatTensor(
                np.random.normal(0, 1, (imgs.shape[0], self.args.latent_dim)))
            # train D
            ...
            if global_steps % self.args.n_critic_search == 0:
                ...
                gen_z = torch.cuda.FloatTensor(
                    np.random.normal(0, 1,
                                     (self.args.gen_bs, self.args.latent_dim)))
            ...
            if gen_step and iter_idx % self.args.print_freq == 0:
                tqdm.write(
                    ...
                    (epoch, self.args.max_epoch_D,
                     iter_idx % len(self.train_loader), len(
    ...
    def search_evol_arch(self, epoch, fid_stat):
        ...
        gen_genotypes = np.concatenate((self.gen_genotypes, offsprings),
                                       axis=0)
        dis_genotypes = np.concatenate((self.dis_genotypes, offsprings_dis),
                                       axis=0)
        ...
        keep, selected = CARS_NSGA(is_values, obj,
                                   keep_N), CARS_NSGA(is_values, obj,
                                                      selected_N)
        for i in selected:
            logger.info(
    ...
    def gen_offspring_dis(self, alphas, offspring_ratio=1.0):
        ...
        while len(offsprings) != n_offspring:
            rand = np.random.rand()
            if rand < 0.5:
                alphas_c = self.mutation_dis(alphas[np.random.randint(
                    0, alphas.shape[0])])
            # elif rand < 0.5:
            else:
                a, b = np.random.randint(0,
